...
void PCSData::readFile(std::string fileName){
	...
	for(int i=0; i<vStr.size(); i++){
	    ...
	    if(Ii < 10.0 || Vi < 10.0) continue;  // I==0, V==0ÇÃÇ±Ç∆Ç™Ç†ÇÈÇÃÇ≈ÅAì«Ç›îÚÇŒÇ∑

	    if(i==1) G.push_back(stod(vStr[i])*1000.0);   // W/m^2íPà Ç…Ç∑ÇÈ
...
/** G,T,I,VÇÃÇRÇOì_ïΩãœÇÇ∆ÇÈ   *****************************************************
 *  ÇΩÇæÇµÅAì˙ÇÕÇ‹ÇΩÇ™Ç»Ç¢ÇÊÇ§Ç…Ç∑ÇÈ ***********************************************/
...
void PCSData::getRandomTimes(MyCalendar myCal[]){
    ...
    for(int i=0; i<=10; i++){
        hour[i] = s[i]/s[10]*7.0;  // ÇPÇUéûÅ[ÇXéûÅÅÇVéûä‘
    }
...
void PCSData::makeSimPCSData05(SimPCSData &sd){
    ...
    double rate {std::exp(std::log(finalRate)/(60*7*3))}; // ÇRì˙Ç≈åJÇËï‘Ç∑
...
/********    ï∂éöóÒstrÇï∂éöcÇ≈ï™äÑÇ∑ÇÈ òAë±ÇµÇΩcÇÕÇ–Ç∆Ç¬Ç∆Ç›Ç»Ç∑    ***************/
